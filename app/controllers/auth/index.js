...
import utils, { buildSuccObject } from "../../middleware/utils";
import uuid from "uuid";
import { addHours } from "date-fns";
import { matchedData } from "express-validator";
import auth from "../../middleware/auth";
import emailer from "../../middleware/emailer";
import phone from "./../../middleware/phone";
...
const findUser = async params => {
  return new Promise((resolve, reject) => {
    User.findOne(
      {
        $or: [
          { $and: [{ email: { $ne: null } }, { email: params.email }] },
          { phone: params.phone }
        ]
      },
 ...
 * @param {string} email - user´s email
 */
const findSocialUser = async params => {
  return new Promise((resolve, reject) => {
    User.findOne(
      {
        $or: [{ socialToken: params.socialToken }, { phone: params.phone }]
      },
      "password loginAttempts blockExpires name email role verified verification"
 ...
 * @param {Object} req - request object
 */
const phoneVerification = async req => {
  return new Promise((resolve, reject) => {
    const verify = new Verify({
      phone: req.phone,
      verification: uuid.v4()
    });
    verify.save((err, item) => {
      if (err) {
        reject(utils.buildErrObject(422, err.message));
      }
      resolve(item);
...
exports.Sociallogin = async (req, res) => {
  try {
    const data = matchedData(req);
    const user = await findSocialUser(data);
    if (user) {
      await userIsBlocked(user);
      await checkLoginAttemptsAndBlockExpires(user);
      // all ok, register access and return token
      ...
      if (!user.socialToken) user.socialToken = data.socialToken;
      await saveLoginAttemptsToDB(user);
      res.status(200).json(await saveUserAccessAndReturnToken(req, user));
    } else {
      res.status(200).json(utils.buildSuccObject("USER_DOES_NOT_EXIST"));
      // const item = await registerUser(data);
      // const userInfo = setUserInfo(item);
      // res.status(200).json(returnRegisterToken(item, userInfo));
    }
  } catch (error) {
    utils.handleError(res, error);
  }
 ...
 * @param {Object} res - response object
 */
exports.VarifyPhone = async (req, res) => {
  try {
    // Gets locale from header 'Accept-Language'
    // const locale = req.getLocale()
    req = matchedData(req);
    const doesEmailExists = await emailer.emailExists(req);
    if (!doesEmailExists) {
      const item = await phoneVerification(req);
...
exports.sendVerificationCode = async (req, res) => {
  try {
    req = matchedData(req);
    const verificationCode = await phone.phoneExists(req);
    if (verificationCode) {
